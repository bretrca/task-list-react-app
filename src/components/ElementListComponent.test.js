...
test("should print the props", async () => {
  ...
  const listTaskSetInputValue = await renderScreen.findByText(
    mockSetInputValue
  );
  const listTaskHandlerUpdate = await renderScreen.findByText(
    mockHandlerUpdate
  );
...
describe("should test all the functions", () => {
  test("should print the prop value and onchange", async () => {
    ...
    await waitFor(() => expect(inputEdit).toHaveBeenCalledTimes(1));
    fireEvent.submit(onChangeElement);
    await waitFor(() => expect(onChangeElement).toHaveBeenCalledTimes(1));
  });
});
